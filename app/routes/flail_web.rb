    ...
    get '/chart_data.json' do
      ...
      (0..23).to_a.reverse.map do |index|
        ...
        if params[:tagged].blank?
          exceptions = FlailException.select("created_at", "filtered_by").within(time, time + 1.hour).unfiltered.take(100)
        else
          exceptions = FlailException.select("created_at", "filtered_by").within(time, time + 1.hour).tagged(params[:tagged]).unfiltered.take(100)
        end
